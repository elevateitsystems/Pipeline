...
export default function SummarySection({
  ...
  const summaryData =
    typeof window !== "undefined" && sessionStorage.getItem("summaryData");
    ...
    if (typeof window !== "undefined") {
      ...
      if (summaryDataStr) {
        try {
          ...
          const normalizedInitialCategories =
            normalizeCategories(categoriesToUse);
  ...
  useEffect(() => {
    ...
    const normalizedSessionCategories = normalizeCategories(
      sessionStorageCategories,
    );
    setCategoryRecommendations((prev) => {
      ...
      normalizedSessionCategories.forEach(
        (cat: { id: string }, index: number) => {
                  ...
                  if (auditDataStr) {
                    try {
                      ...
                      if (
                        ...
                        return auditData.categories.map(
                          (cat: { id: string; name: string }) => ({
                            id: cat.id,
                            name: cat.name || `Category ${cat.id}`,
                          }),
  ...
  const [refreshedCategories, setRefreshedCategories] = useState<
